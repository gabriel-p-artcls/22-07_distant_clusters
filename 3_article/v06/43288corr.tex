...
%
\usepackage{hyperref}
\hypersetup{colorlinks=true, urlcolor=blue, citecolor=cyan}
...
\section{Introduction}
 ...
 played an important role in determining cluster parameters.\LEt{ the way can't   be massive.   }
 The continuous increase of high quality data is a \LEt{ defy=resist. defining? } circumstance
 ...
 explain below.\LEt{ The slash "/" is used to indicate a ratio and some instrument pairings. Please check throughout: you can substitute "and", "or", "and/or", or a double hyphen (which can be used to indicate dual nature: Hertzprung--Russell diagram).
 }
...
\LEt{ yes? All abbreviations and acronyms (and instrument and program names when appropriate) must be introduced at first use; the abbreviation should then be used consistently. Please check throughout.
 ...
 non-trivial sequences in the photometric space.\LEt{ in other words, the clusters you are testing are composed of a small number of members with non-t. sequences (not the surrounding stars). }\\
...
\section{Catalogs, clusters, and data}
 ...
 \begin{table*}
 \caption{Selected open clusters with a cataloged distance $\geq$9000 pc,
 ordered by right ascension. The ages are expressed as the logarithm, and the
 distances are in parsec. The short names used for the clusters
 throughout the article in tables and figures are in parentheses. Clusters with no distances below
 the 9000 pc limit in any of the catalogs are  in bold.}
 \label{tab:clusters}
 \centering
 ...
 the four catalogs, positioned on the face-on view of the Galaxy\LEt{ To follow IAU recommendations, the words Galaxy and Galactic should be in upper case when they refer to our Galaxy, and galaxy and galactic in lower case when they refer to another (or generic) galaxy. Please check throughout: You have used both in this paper, and  I am not always able to tell the difference.
 } (top), and
 ...
 \begin{figure*}
  ...
  \caption{\LEt{ Please start with a brief description of the figure as a whole before describing the individual panels. }Left: Position of the 25 clusters selected from the four
...
\section{Cluster analysis}
 ...
 \subsection{Structural analysis}
  ...
  tidal radii ($r_{c}$, $r_{t}$).\LEt{ FYI: When the abbreviation is  made of the first letter of each word, "hereafter" is not necessary; when the abbreviation is not clear from the context or for  authors and papers, use "hereafter" before the abbreviation: In Jones \&\ Brown (2015, hereafter JB15) ... . Hereinafter is also used, but A\&A prefers "hereafter". \      } The adopted radius $r_{a}$ is the limiting
  ...
  remove projection artifacts. \LEt{ Legend information, colors, lines, etc., should  be in the figure caption and should not be repeated in the main text unless necessary to understand the discussion. Check for this throughout the paper, and please remove where possible.  
 }The right plot shows the radial density analysis
  ...
  \begin{figure*}
   ...
   \caption{\LEt{ description }Left: Analyzed $20^{\prime} \times 20^{\prime}$ arcmin frame with the
   ...
   logarithmic axis (see text for details).\LEt{ see note 13. The first sentence of each figure legend should be a descriptive title, omitting the initial article (The, A, An). The following text should concisely label and explain the figure and any panels. In other words, someone looking only at the figure and its caption should be able to understand what it represents. Any discussion or opinion should go in the main text. Please check throughout.}}
 ...
 \subsection{Membership and fundamental parameters}
  ...
  \LEt{ The specific steps you took for this particular research should be in the past simple: We used, We extrapolated, The observation was performed, etc. Descriptions of methodology, universal truths, constants, or findings should be in the present simple: The first step in the process is, Water boils at 100^{\circ}$C, We find, etc. I'm not always able to tell the difference. Please check throughout. }Before we can estimate the fundamental parameters with \texttt{ASteCA}, we
  ...
  \begin{figure*}
   ...
   \caption{\LEt{ description }Left: VPD for stars in the analyzed Berkeley 29
...
\section{Results and discussion}
 ...
 \end{itemize}
  ...
  clusters\LEt{ i.e., only for the fourth run. If this is for all four runs, place the phrase before the ":" }~\citep{vonHippel_2005}.
  ...
  \begin{figure}
   ...
   \caption{\LEt{ description }Top: Distances estimated with the main run of \texttt{ASteCA}
  ...
  \begin{figure}
   ...
   \caption{\LEt{ description }Top: Differences in logarithmic ages between \texttt{ASteCA} and
  ...
  as stated previously), it is a bit high.\LEt{ FYI: Generally, for rates, ratios,  percentages, and masses use high(er) and low(er); for sizes and magnitudes use large(r) and small(er).
 } On the other hand, the
  uncertainties are also rather large and the binary fractions assigned to
  most of the clusters drop below 50\% within their 1$\sigma$\LEt{ one sigma } range.
...
\section{Conclusions}
 \label{sec:conclusions}
  ...
  whose maximum height above the galactic equator takes place\LEt{ is located? } at about
...
\section{Cluster by cluster discussion}
 \label{app:indiv_clusters}
  ...
  giant branch is even less notorious.\LEt{ =widely and unfavorably known (a notorious criminal). perhaps well known?   }
  ...
  sample.\LEt{ the cluster with the second largest number  } It is also the second most massive cluster in our sample according
